...
This version applies the uv.jpeg texture to replace the flat green color
"""
import cv2
import numpy as np
import math
...
def draw_textured_3d_model(frame, vertices, faces, projected_vertices, texture_data, texture_image):
    """Draw 3D model faces with UV texture mapping"""
            ...
            if len(face) >= 3:
                ...
                if valid_face and len(face_points) >= 3:
                    ...
                    if (face_tex_indices is not None and 
                        ...
                        # Simple texture mapping: sample texture at triangle center
                        if len(uv_coords) >= 3:
                            ...
                            texture_color = sample_texture(texture_image, avg_u, avg_v)
                        else:
                            texture_color = (20, 100, 40)  # Fallback color
                        
                        # Draw textured face
                        cv2.fillPoly(result_frame, [face_points], texture_color)
                        
                        # Optional: Draw subtle wireframe
                        edge_color = tuple(max(0, int(c * 0.8)) for c in texture_color)  # Darker edge
                        cv2.polylines(result_frame, [face_points], True, edge_color, 1)
                        
                    else:
                        # Fallback: draw with default breadboard color
                        breadboard_color = (20, 100, 40)  # Dark green-brown
                        cv2.fillPoly(result_frame, [face_points], breadboard_color)
...
def ar_main_textured():
    print("Textured AR Visualizer - Realistic Breadboard with UV Mapping")
            ...
            if ids is not None:
                ...
                for i, corner in enumerate(corners):
                    ...
                    if success:
                        ...
                        # Draw textured 3D breadboard model
                        if use_texture:
                            ...
                            from ar_modern_ui import draw_3d_model
